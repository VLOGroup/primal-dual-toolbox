...
/** \brief Parameters for the TgvMriOptimizer optimizer.
 */
template<typename PixelType>
struct PRIMALDUALTOOLBOX_DLLAPI TgvMriParameters
{
  /** Constructor. */
  TgvMriParameters();
  ...
  friend std::ostream& operator<<(std::ostream& msg,
                                  const TgvMriParameters<PixelType>& params)
  {
    return msg << params.print();
  }
};

/** \brief TGV optimizer for MRI reconstruction.
 *
 *   According to Matlab code of
...
class PRIMALDUALTOOLBOX_DLLAPI TgvMriOptimizer
{
private:
...
public:
  ...
  /** Get optimizer parameters of type TgvMriParameters. */
  TgvMriParameters<real_type>& getParameters()
  {
    return params_;
  }

  /** No copies are allowed. */
  TgvMriOptimizer(TgvMriOptimizer const&) = delete;

  /** No assignments are allowed. */
  void operator=(TgvMriOptimizer const&) = delete;

private:
  /** Optimizer parameters of type TgvMriParameters. */
  TgvMriParameters<real_type> params_;
