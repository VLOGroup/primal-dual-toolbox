...
/** \brief Parameters for the TvOptimizer optimizer.
 */
template<typename PixelType>
struct PRIMALDUALTOOLBOX_DLLAPI TvParameters
{
  /** Constructor. */
  TvParameters();
  ...
  friend std::ostream& operator<<(std::ostream& msg,
                                  const TvParameters<PixelType>& params)
  {
    return msg << params.print();
  }
};

/** Total-Variation optimizer for denoising. */
template<typename InputType, typename OutputType>
class PRIMALDUALTOOLBOX_DLLAPI TvOptimizer
{
private:
...
public:
  ...
  TvOptimizer(TvOptimizer const&) = delete;

  /** No assignments are allowed. */
  void operator=(TvOptimizer const&) = delete;
