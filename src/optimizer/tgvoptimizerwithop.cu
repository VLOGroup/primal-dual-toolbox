...
TgvOptimizerWithOp<InputType, OutputType>::TgvOptimizerWithOp() :
    op_(NULL)
{
}
...
template<typename InputType, typename OutputType>
InputType* TgvOptimizerWithOp<InputType, OutputType>::getResult()
{
	if (u_ == nullptr)
	{
		// throw exception
...
template<typename InputType, typename OutputType>
void TgvOptimizerWithOp<InputType, OutputType>::setOperator(
...
template<typename InputType, typename OutputType>
void TgvOptimizerWithOp<InputType, OutputType>::setInput0(const InputType &input)
{
  // primal variables
...
template<typename InputType, typename OutputType>
void TgvOptimizerWithOp<InputType, OutputType>::setNoisyData(
...
template<typename InputType, typename OutputType>
void TgvOptimizerWithOp<InputType, OutputType>::solve(bool verbose)
{
  if (op_ == nullptr)
  {
    // throw exception
  ...
  for (unsigned int k = 0; k <= params_.max_iter; k++)
  {
    if (!(k % params_.check))
    {
      std::cout << "iter=" << k << " alpha0=" <<  params_.alpha0 << " alpha1=" << params_.alpha1 << " lambda=" << params_.lambda << std::endl;
    }
...
template class TgvOptimizerWithOp<iu::LinearDeviceMemory<float2, 2>,
    iu::LinearDeviceMemory<float2, 3>> ;
template class TgvOptimizerWithOp<iu::LinearDeviceMemory<float2, 2>,
    iu::LinearDeviceMemory<float2, 2>> ;
template class TgvOptimizerWithOp<iu::LinearDeviceMemory<float, 2>,
    iu::LinearDeviceMemory<float, 3>> ;
template class TgvOptimizerWithOp<iu::LinearDeviceMemory<double2, 2>,
    iu::LinearDeviceMemory<double2, 3>> ;
template class TgvOptimizerWithOp<iu::LinearDeviceMemory<double2, 2>,
    iu::LinearDeviceMemory<double2, 2>> ;
template class TgvOptimizerWithOp<iu::LinearDeviceMemory<float, 2>,
    iu::LinearDeviceMemory<float, 2>> ;
template class TgvOptimizerWithOp<iu::LinearDeviceMemory<double, 2>,
