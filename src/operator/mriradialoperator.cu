...
template<typename InputType, typename OutputType>
MriRadialSinglecoilOperator<InputType, OutputType>::MriRadialSinglecoilOperator(const OpConfigDict &config) :
    OperatorBase<InputType, OutputType>(2, "MriRadialOperator", config, 4, 4)
{
  osf_ = this->getConfigDouble("osf");
  sector_width_ = this->getConfigInt("sector_width");
  kernel_width_ = this->getConfigInt("kernel_width");
...
template<typename InputType, typename OutputType>
MriRadialSinglecoilOperator<InputType, OutputType>::~MriRadialSinglecoilOperator()
{
}

template<typename InputType, typename OutputType>
void MriRadialSinglecoilOperator<InputType, OutputType>::sizeCheck(
    const InputType & src, const OutputType & dst)
{
  IU_SIZE_CHECK(src.size(), iu::Size<2>({img_dim_, img_dim_}));
  const int nCh = dst.size()[1];
...
template<typename InputType, typename OutputType>
iu::Size<InputType::ndim> MriRadialSinglecoilOperator<InputType, OutputType>::getInputSize(const OutputType& output)
{
  iu::Size<2> input_size({img_dim_, img_dim_});
  return input_size;
}
...
template<typename InputType, typename OutputType>
iu::Size<OutputType::ndim> MriRadialSinglecoilOperator<InputType, OutputType>::getOutputSize(const InputType& input)
{
  auto iu_trajectory = this->template getConstant<real_type, 2>(0);
  const int kspace_dim = iu_trajectory->size()[0];
  iu::Size<2> output_size({kspace_dim, 1});
  return output_size;
}

template<typename InputType, typename OutputType>
void MriRadialSinglecoilOperator<InputType, OutputType>::createNufftOperator()
{
  // extract constants
  auto iu_trajectory = this->template getConstant<real_type, 2>(0);
  auto iu_dcf = this->template getConstant<real_type, 2>(1);
  ...
  gpuNUFFT::GpuNUFFTOperatorFactory factory(true,true,true);
  nufft_op_.reset(factory.createGpuNUFFTOperator(trajectory, dcf, kernel_width_, sector_width_, osf_, img_dims));
}

template<typename InputType, typename OutputType>
void MriRadialSinglecoilOperator<InputType, OutputType>::executeForward(const InputType & src,
                                                          OutputType & dst)
{
  if (nufft_op_ == nullptr)
  {
...
template<typename InputType, typename OutputType>
void MriRadialSinglecoilOperator<InputType, OutputType>::executeAdjoint(
    const OutputType & src, InputType & dst)
{
  if (nufft_op_ == nullptr)
  {
...
template<typename InputType, typename OutputType>
void MriRadialSinglecoilOperator<InputType, OutputType>::adjointnessCheck()
{
  iu::Size<2> u_size({img_dim_, img_dim_});
  iu::Size<2> p_size({this->template getConstant<real_type, 2>(1)->size()[0], 1});
  InputType u(u_size);
  OutputType p(p_size);
  iu::random::fillRandomFloatingNumbers(u);
  iu::random::fillRandomFloatingNumbers(p);
...
// explicit template instantiations
template class MriRadialSinglecoilOperator<iu::LinearDeviceMemory<float2, 2>,
    iu::LinearDeviceMemory<float2, 2>> ;
//template class MriRadialOperator<iu::LinearDeviceMemory<double2, 2>,
//    iu::LinearDeviceMemory<double2, 2>> ;
