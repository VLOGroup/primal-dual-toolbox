...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 3)
    mexErrMsgIdAndTxt(
        "MATLAB:gpuMriCartesianRemoveROOSAdj:invalidNumInputs",
        "Three inputs required (sensitivities, rawdata, mask)");
  if (nlhs > 1)
    mexErrMsgIdAndTxt("MATLAB:gpuMriCartesianRemoveROOSAdj:maxlhs",
                      "Too many output arguments.");

  // Check datatypes
  if( !mxIsDouble(prhs[0]) || !mxIsComplex(prhs[0]) )
  {
      mexErrMsgIdAndTxt("MATLAB:gpuMriCartesianRemoveROOSAdj:wrongDataType",
