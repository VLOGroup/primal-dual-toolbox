...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 3)
    mexErrMsgIdAndTxt(
        "MATLAB:gpuMriCartesianRemoveROOSFwd:invalidNumInputs",
        "Three inputs required (sensitivities, rawdata, mask)");
  if (nlhs > 1)
    mexErrMsgIdAndTxt("MATLAB:gpuMriCartesianRemoveROOSFwd:maxlhs",
                      "Too many output arguments.");

  // Check datatypes
  if( !mxIsDouble(prhs[0]) || !mxIsComplex(prhs[0]) )
  {
      mexErrMsgIdAndTxt("MATLAB:gpuMriCartesianRemoveROOSFwd:wrongDataType",
  ...
  iu::LinearHostMemory<complex_type, 2> h_image(*prhs[1]);
  iu::LinearHostMemory<real_type, 2> h_mask(*prhs[2]);

  // do something magical

  // Init operator
  std::shared_ptr < MriCartesianRemoveROOSOperator<InputType, OutputType>
      > op(new MriCartesianRemoveROOSOperator<InputType, OutputType>);
  ...
  // Copy host to device
  iu::LinearDeviceMemory<complex_type, 2> d_image(h_image.size());
  iu::copy(&h_image, &d_image);

  iu::Size<3> output_size = op->getOutputSize(d_image);

  // Get result
  iu::LinearDeviceMemory<complex_type, 3> d_output(output_size);
  iu::LinearHostMemory<complex_type, 3> h_output(output_size);

  op->forward(d_image, d_output);
