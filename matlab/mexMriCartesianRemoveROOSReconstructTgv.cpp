...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 4)
    mexErrMsgIdAndTxt(
        "MATLAB:gpuTgvMriOptimizer_cartesianRemoveROOS:invalidNumInputs",
        "Four inputs required (sensitivities, rawdata, mask, tgv_params)");
  if (nlhs > 1)
    mexErrMsgIdAndTxt("MATLAB:gpuTgvMriOptimizer_cartesianRemoveROOS:maxlhs",
                      "Too many output arguments.");

  // Check datatypes
  if( !mxIsDouble(prhs[0]) || !mxIsComplex(prhs[0]) )
  {
      mexErrMsgIdAndTxt("MATLAB:gpuTgvMriOptimizer_cartesianRemoveROOS:wrongDataType",
