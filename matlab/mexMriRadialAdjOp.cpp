...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 5)
    mexErrMsgIdAndTxt(
        "MATLAB:gpuMriRadialAdj:invalidNumInputs",
        "Five inputs required (trajectory, dcf, sensitivities, rawdata, nufft_params)");
  if (nlhs > 1)
    mexErrMsgIdAndTxt("MATLAB:gpuMriRadialAdj:maxlhs",
                      "Too many output arguments.");

  // Check datatypes
  if( !mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]) )
  {
      mexErrMsgIdAndTxt("MATLAB:gpuMriRadialAdj:wrongDataType",
  ...
  // Copy host to device
  iu::LinearDeviceMemory<complex_type, 2> d_rawdata(h_rawdata.size());
